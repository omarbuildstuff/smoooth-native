 ...
 */
function createTray() {
  const icon = nativeImage.createFromPath(path.join(VITE_PUBLIC, 'smoooth-appicon-tray.png'))
  ...
  appState.tray.setToolTip('Smoooth is recording...')
...
export async function cleanupOrphanedRecordings() {
  ...
  try {
    ...
    const filePattern = /^Smoooth-recording-\d+(-screen\.mp4|-screen\.muxed\.mp4|-screen\.webm|-webcam\.mp4|-system\.webm|-mic\.m4a|\.json)$/
